...
final class EncryptedMsBackupArchive
{
    ...
    private function decryptToTemporary(
        ...
        try {
            ...
            if ($magic !== self::MAGIC) {
                throw new BackupArchiveException('The selected file is not a supported encrypted MediSmart backup.');
            }
    ...
    private function assertCryptographyAvailable(): void
    {
        if (! extension_loaded('sodium')
            ...
            throw new BackupArchiveException('The Sodium extension is required for encrypted MediSmart backups.');
        }

        if (PHP_INT_SIZE < 8) {
            throw new BackupArchiveException('Encrypted MediSmart backups require a 64-bit PHP runtime.');
        }
    }
    ...
    private function readableArchivePath(string $path, bool $requireExtension = true): string
    {
        ...
        if (! is_string($resolved) || ! is_file($resolved) || ! is_readable($resolved)
            ...
            throw new BackupArchiveException('The selected file is not a readable MediSmart backup.');
        }
